...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Find superintendent or district contact emails for Michigan zero-score districts.")
    parser.add_argument(
        "--input",
        dest="input_path",
        default="",
    ...
    parser.add_argument(
        "--output",
        dest="output_path",
        default="scrapers/mi_contacts_output.csv",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Only process the first N Michigan zero-score districts.",
    )
...
def load_mi_zero_score_districts(dataset_path: Path, limit: int) -> List[Dict[str, str]]:
    with dataset_path.open(newline="", encoding="utf-8") as handle:
        rows = [
            row
            for row in csv.DictReader(handle)
            if (row.get("state") or "").strip() == "MI" and (row.get("index_score") or "").strip() == "0"
        ]
    if limit > 0:
        return rows[:limit]
    return rows
...
def extract_page_data(page_url: str, html_bytes: bytes) -> Tuple[str, List[Tuple[str, str]], Set[str]]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if href.lower().startswith(("tel:", "javascript:")):
            continue
        internal_links.append((normalize_url(page_url, href), anchor_text))
...
def main() -> int:
    ...
    try:
        ...
        rows = load_mi_zero_score_districts(dataset_path, args.limit)
    ...
    for index, row in enumerate(rows, start=1):
        district = (row.get("district") or "").strip()
        print(f"[{index}/{len(rows)}] {district}", file=sys.stderr)
